...
class AutomationTab(ttk.Frame):
    ...
    def _build(self):
        ...
        ttk.Label(image_frame, text="Ảnh giới hạn tìm kiếm #2 (App):").grid(row=4, column=0, sticky="w", pady=2)
        ttk.Entry(image_frame, textvariable=self.state["image_path_vars"]["app_ratelimit_path_2"], state="readonly").grid(row=4, column=1, padx=5, sticky="ew")
        ttk.Button(image_frame, text="Tải ảnh...", command=lambda: self.callbacks["handle_load_fail_image"]("app_ratelimit_path_2")).grid(row=4, column=2)

        ttk.Label(image_frame, text="Ảnh giới hạn tìm kiếm #2 (Web):").grid(row=5, column=0, sticky="w", pady=2)
        ttk.Entry(image_frame, textvariable=self.state["image_path_vars"]["web_ratelimit_path_2"], state="readonly").grid(row=5, column=1, padx=5, sticky="ew")
        ttk.Button(image_frame, text="Tải ảnh...", command=lambda: self.callbacks["handle_load_fail_image"]("web_ratelimit_path_2")).grid(row=5, column=2)

        ttk.Label(image_frame, text="Ảnh báo thành công (App):").grid(row=6, column=0, sticky="w", pady=2)
        ttk.Entry(image_frame, textvariable=self.state["image_path_vars"]["app_success_path"], state="readonly").grid(row=6, column=1, padx=5, sticky="ew")
        ttk.Button(image_frame, text="Tải ảnh...", command=lambda: self.callbacks["handle_load_fail_image"]("app_success_path")).grid(row=6, column=2)

        ttk.Label(image_frame, text="Ảnh báo thành công (Web):").grid(row=7, column=0, sticky="w", pady=2)
        ttk.Entry(image_frame, textvariable=self.state["image_path_vars"]["web_success_path"], state="readonly").grid(row=7, column=1, padx=5, sticky="ew")
        ttk.Button(image_frame, text="Tải ảnh...", command=lambda: self.callbacks["handle_load_fail_image"]("web_success_path")).grid(row=7, column=2)
